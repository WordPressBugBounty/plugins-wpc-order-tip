...
class Wpcot_Backend {
    ...
    public function enqueue_scripts( $hook ) {
        if ( str_contains( $hook, 'wpcot' ) ) {
            ...
            wp_localize_script( 'wpcot-backend', 'wpcot_vars', [
                    'nonce'       => wp_create_nonce( 'wpcot_backend' ),
    ...
    public function admin_menu_content() {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only URL parameter for tab navigation.
        ...
        <div class="wpclever_settings_page wrap">
            ...
            <h2></h2>
            <?php
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only notice flag generated by WordPress options API.
    ...
    public function ajax_add_tip() {
        if ( ! check_ajax_referer( 'wpcot_backend', 'nonce', false ) || ! current_user_can( 'manage_options' ) ) {
            wp_die();
        }
